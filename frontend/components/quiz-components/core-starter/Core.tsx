...
import ShowResultCard from "../ShowResult/ShowResultCard";
...
const Core = function ({
    ...
    const renderAnswerInResult = (question: { answers?: any; correctAnswer?: any; questionType?: any; answerSelectionType?: any; }, userInputIndex: string | any[]) => {
        ...
        return answers.map((answer: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined, index: React.Key | null | undefined) => {
    ...
    const renderQuizResultQuestions = useCallback(() => {
        ...
        let filteredUserInput: never[];

        if (filteredValue !== "all") {
            if (filteredValue === "correct") {
                // @ts-ignore
                filteredQuestions = questions.filter((question: any, index: any) => correct.indexOf(index) !== -1);
                // @ts-ignore
                filteredUserInput = userInput.filter((input, index) => correct.indexOf(index) !== -1);
            } else {
                // @ts-ignore
                filteredQuestions = questions.filter((question: any, index: any) => incorrect.indexOf(index) !== -1);
                // @ts-ignore
                filteredUserInput = userInput.filter((input, index) => incorrect.indexOf(index) !== -1);
            }
        }

        return (filteredQuestions || questions).map((question: { answerSelectionType: any; questionIndex?: any; question?: any; questionPic?: any; correctAnswer: any; segment?: any; answers?: any; questionType?: any; }, index: number) => {
            ...
            return (
                <div className="result-answer-wrapper" key={index + 1}>
                    ...
                    {question.questionPic &&
                        <img className="rounded-[4rem] pl-4 pr-4" src={question.questionPic} alt="image"/>}
    ...
    const renderAnswers = (question: { answers?: any; correctAnswer?: any; questionType?: any; questionIndex?: any; answerSelectionType?: any; }, buttons: { [x: string]: { className: any; }; }) => {
        const {
            answers, correctAnswer, questionType, questionIndex,
        } = question;
        let {answerSelectionType} = question;
        const onClickAnswer = (index: number) => checkAnswer(index + 1, correctAnswer, answerSelectionType, {
            userInput,
            userAttempt,
            currentQuestionIndex,
            continueTillCorrect,
        ...
        const onSelectAnswer = (index: number) => selectAnswer(index + 1, correctAnswer, answerSelectionType, {
            userInput,
            currentQuestionIndex,
            setButtons,
            setShowNextQuestionButton,
    ...
    const renderTags = (answerSelectionType: string | undefined, numberOfSelection: any, segment: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined) => {
        const {
                ...
                && (
                    <div className="questionWrapperBody bg">
                        ...
                        {question && question.questionPic &&
                            <img className="p-2.5 rounded-[4rem]" src={question.questionPic} alt="image"/>}
