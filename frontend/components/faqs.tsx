...
import FAQ from "@/generated/com/example/application/entities/faq/FAQ";

type faqAccordionProps = {
    faqs: FAQ[],
    showSupportButton: boolean
}
export default function Accordion({faqs, showSupportButton}: faqAccordionProps) {
    const half = Math.ceil(faqs.length / 2);
    const firstHalf = faqs.slice(0, half);
    const secondHalf = faqs.slice(half);
    return (
        <section
            className="relative z-20 overflow-hidden bg-white pt-20 pb-12 lg:pt-[120px] lg:pb-[90px] wavy-faq-section">
...
const AccordionItem = ({header, text}: faqProps) => {
    ...
    return (
        <div className="single-faq mb-8 w-full rounded-3xl border border-[#F3F4FE]
         bg-white p-4 sm:p-8 lg:px-6 xl:px-8 shadow-2xl">
            ...
            >
                <div
                    className="mr-5 flex h-10 w-full max-w-[40px] items-center justify-center rounded-lg bg-white bg-opacity-5 text-primary">
                    <svg
